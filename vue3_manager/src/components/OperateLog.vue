    ...
    >
      ...
      >
        <template #default="scope">
          <el-tag
              :color="gettabcolor(scope.row.identify)"
              size="large"
              disable-transitions
          >{{ scope.row.identify }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="operate" label="操作内容" width="250" align="center"></el-table-column>
      <el-table-column prop="ip" label="IP" align="center"></el-table-column>
      <el-table-column prop="operatedate" sortable label="操作时刻" align="center"></el-table-column>
...
const gettabcolor=(tag)=> {
  // 根据标签获取颜色
  return tagscolor.value[tag];
}
...
const gettablemsg=()=>{
  request.post("/getlog").then(result => {
    if (result.code === 1) {
      result.data.forEach(item => {
        // 原始的 UTC 时间戳
        const utcTimestamp = item.operatedate;
        // 创建 Date 对象并将 UTC 时间戳传入
        const utcDate = new Date(utcTimestamp);
        // 获取本地时间
        const localDate = new Date(utcDate.getTime());
        // 获取本地时间的字符串表示
        item.operatedate = localDate.toLocaleString();
...
const select=()=>{
  ...
  if(searchflag.value==="0")
  {
    tableShow.value=tableData.value.filter(data => {
      return (!search.value || data.userid.toString().toLowerCase().includes(search.value.toLowerCase()))
    })
  }
  else
  {
    tableShow.value=tableData.value.filter(data => {
      return (!search.value || data.operate.toString().toLowerCase().includes(search.value.toLowerCase()))
    })
  }
  tabletotal.value=tableShow.value.length
}
...
const handleDelete=(row)=>{
  // 弹出消息提示
  ElMessageBox.confirm('此操作将永久删除该操作日志, 是否继续?', '警告', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    //删除日志
    request.post("/deletelog", {
      "id": row["id"]
    }).then(result => {
      if (result.code === 1) {
