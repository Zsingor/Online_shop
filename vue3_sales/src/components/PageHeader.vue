<template>
  ...
  >
    ...
    <el-sub-menu v-if="hasUsername()" index="1"  style="margin-left: auto">
      <template #title><el-icon><User /></el-icon>{{ salesid }}</template>
...
var salesid=ref(JSON.parse(localStorage.getItem("username")))

const hasUsername = () => {
  salesid.value=JSON.parse(localStorage.getItem("username"))
...
const deleteaccount=()=>{
  ...
  ElMessageBox.confirm(
    ...
    request.post("/salesdelete", {
      "salesid": JSON.parse(localStorage.getItem("username"))
    }).then(result => {
      console.log(result)
      if (result.code === 1) {
